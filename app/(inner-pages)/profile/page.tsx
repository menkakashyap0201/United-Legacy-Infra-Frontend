...
function StatusBadge({ status }: { status: number }) {
  ...
  return (
    <span
      className={`inline-flex shrink-0 items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-bold ${active ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}
    >
      <span
  ...
  return (
    <div>
      ...
      <div className={`${boxCls} border-[#0B1D45]/12`}>
        ...
        <div className="flex min-w-0 flex-1 items-center gap-1 py-3 text-sm font-semibold text-[#0B1D45]">
          {children}
        </div>
      </div>
    </div>
...
function AccountRow({ account }: { account: Account }) {
  return (
    <li
      className={`flex items-center gap-3 rounded-xl border bg-white px-3.5 py-3 ${account.is_current ? "border-[#D4A437] shadow-[0_0_0_4px_rgba(212,164,55,.12)]" : "border-[#0B1D45]/12"}`}
    >
      ...
      <div className="min-w-0 flex-1 text-left">
        <p className="flex items-center gap-2 text-sm font-bold text-[#0B1D45]">
          ...
          {account.is_current && (
            <span
              className={`${goldGrad} shrink-0 rounded-full px-2 py-0.5 text-[10px] font-bold text-[#0B1D45]`}
            >
              Current
            </span>
          )}
...
function ProfileContent() {
  ...
  const current =
    data?.accounts.find((a) => a.is_current) ??
    data?.accounts.find((a) => a.referal_code === data?.current);
  const name = current?.name ?? localUser?.name ?? "";
  const code =
    current?.referal_code ?? data?.current ?? localUser?.referal_code ?? "";
  ...
  return (
    <>
      <PageTitle
        title={
          <>
            My <span className={goldText}>profile</span>
          </>
        }
        sub="Your account details and linked IDs."
      />
      ...
      {!loading && error && (
        <div
          role="alert"
          className="grid gap-3 rounded-xl border border-rose-200 bg-rose-50 px-4 py-5 text-center"
        >
      ...
      {!loading && !error && data && (
        <div className="flex flex-1 flex-col">
          ...
          <div className="flex flex-col items-center text-center">
            <span
              className={`${goldGrad} grid h-20 w-20 place-items-center rounded-full text-2xl font-extrabold text-[#0B1D45] shadow-[0_0_40px_rgba(212,164,55,.45)]`}
            >
              {initials(name)}
            </span>
            <h2
              className={`${serif.className} mt-3 text-2xl font-bold text-[#0B1D45]`}
            >
              {name || "—"}
            </h2>
            {localUser?.email && (
          ...
          <div className="mt-5 grid gap-3">
            {code && (
              <InfoBox label="REFERRAL CODE" icon={FaGift}>
                <span className="min-w-0 flex-1 truncate font-mono tracking-wider">
                  {code}
                </span>
            ...
            {current && (
              <div className="grid grid-cols-2 gap-3">
                <InfoBox label="PLC" icon={FaSitemap}>
                  {positionLabel(current.team_position)}
                </InfoBox>
                <InfoBox label="JOINED" icon={FaCalendarDays}>
            ...
            <div>
              <div className="mb-1 flex items-center justify-between">
                <p className={`${labelCls} !mb-0 flex items-center gap-1.5`}>
                  <FaUserGroup className="h-3 w-3" /> MY ACCOUNTS
                  <span className="rounded-full bg-[#0B1D45]/8 px-1.5 py-0.5 text-[10px] tracking-normal text-[#0B1D45]/60">
                ...
                {/* <Link
                  ...
                  className={`${goldGrad} group inline-flex items-center gap-1.5 rounded-full py-1.5 pl-2 pr-3 text-[11px] font-extrabold text-[#0B1D45] shadow-[0_8px_18px_-8px_rgba(212,164,55,.9)] transition hover:-translate-y-0.5`}
                >
                  <span className="grid h-5 w-5 place-items-center rounded-full bg-[#0B1D45] text-[#F5D06B]">
                    <FaArrowRightArrowLeft className="h-2.5 w-2.5 transition-transform duration-300 group-hover:rotate-180" />
                  </span>
                  Switch
                </Link> */}
              </div>
          ...
          <div className="mt-auto grid gap-3 pt-6">
            <Link href="/switch-user" className={btnNavy}>
              <FaArrowRightArrowLeft className="h-3.5 w-3.5" /> Switch account
            </Link>
